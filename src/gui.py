...
def aboutpage() -> None:
    ...
    version_text = tkinter.Label(about_page, font=("Segoe UI Light", 21), fg="#0000bb", text="Spam Bot GUI")
    version_text.place(x=130, y=0)

    version_info = tkinter.Label(about_page, font=("Consolas", 9), text=f"version: {VERSION}", relief="flat")
    version_info.place(x=125, y=46)
    about_page.update_idletasks()
    version_info.place_configure(x=(version_text.winfo_width() - version_info.winfo_width()) / 2 + 130)
    ...
    copyright_info = tkinter.Label(about_page, font=("TkDefaultFont", 10), text="Copyright (c) 2020 eric15342335.")
    copyright_info.place(x=125, y=70)
    copyright_info2 = tkinter.Label(about_page, font=("TkDefaultFont", 10), text="All Rights Reversed.")
    copyright_info2.place(x=125, y=95)

    github_page = tkinter.Label(about_page, text="Github Repository", bg='#00ffee')
    github_page.bind("<Button-1>",
    ...
    dont_click.bind("<Button-1>",
                    lambda e: webbrowser.open('https://www.youtube.com/watch?v=dQw4w9WgXcQ', new=0, autoraise=True))
...
def clipboard_check() -> None:
    ...
    clipboard_display.resizable(0, 0)
    ...
    clipboard_list = tkinter.Listbox(clipboard_display,
                                     activestyle="none",
    ...
    def cb_experimental_msg() -> None:
        ...
        clipboard_list.insert(tkinter.END, f"Current clock: {time.strftime('%H:%M:%S', time.localtime())}",
        ...
        try:
            ...
            if server_get.status_code != 200:
                clipboard_list.insert(tkinter.END, f'Status code {server_get.status_code}, do the repo exist?')
            else:
                ...
                def connect_initialize() -> None:
                    try:
                        ...
                        if 0 < selected < len(list(server_get)) + 1:
                            ...
                            address, port = selected_server["address"], selected_server["port"]
                            try:
                            ...
                            try:
                                ...
                                clipboard_list.insert(tkinter.END, "Successfully connected to the server.")
                                ...
                                def receivemsg() -> None:
                                    ...
                                    while True:
                                        try:
                                            ...
                                            clipboard_list.insert(tkinter.END, message.decode(utils.ENCODING))
                                ...
                                sock.send(f"{utils.return_time(False)}[{username}]{utils.PING_MESSAGE}".encode(
                                    utils.ENCODING))
                                ...
                                def disconnect() -> None:
                                    ...
                                    try:
                                        sock.close()
                                        cb_exp_msg_button.configure(text="Disconnected", command=time.sleep(0.1))
                                ...
                                def sentmsg_sent() -> None:
                                    ...
                                    send_message_button.configure(text="Send", command=time.sleep(0.1))
                                    clipboard_display.update_idletasks()
                                    try:
                                        sock.send(f"{utils.return_time(False)}[{username}]{message_sent.get()}".encode(
                                            utils.ENCODING))
                                    except socket.error as sending_error:
                                        clipboard_list.insert(tkinter.END, "Cannot send message to server.")
                                        clipboard_list.yview(tkinter.END)
                                        clipboard_list.insert(tkinter.END, str(sending_error))
                                        for line_send_error in list(traceback.format_exc().split("\n")):
                                            clipboard_list.insert(clipboard_list.size() + 1, line_send_error)
                                ...
                                clipboard_list.insert(tkinter.END, "Connection Failed. Server is not online: timeout")
                        else:
                ...
                send_message_button = tkinter.Button(clipboard_display, text="Connect", command=connect_initialize)
                ...
                clipboard_list.insert(tkinter.END, f"Enter {list(range(1, len(server_get) + 1))} "
    ...
    cb_exp_msg_button = tkinter.Button(clipboard_display, text="Experimental Chatroom", command=cb_experimental_msg)
...
def check_update() -> None:
    ...
    update_info = tkinter.Listbox(update,
                                  activestyle='none',
    ...
    try:
        ...
        if newest_version.status_code != 200:
            update_info.insert(tkinter.END, f"Status code {newest_version.status_code}, do the repo exist?")
        else:
            ...
            if newest_version[utils.GUI]['version'] != VERSION:
                update_info.insert(tkinter.END, "", f"New version {newest_version[utils.GUI]['version']} is available!")
            else:
                update_info.insert(tkinter.END, "Program is up-to-date!")
    except OSError as check_error:
        update_info.insert(tkinter.END, check_error)
...
def keylogger() -> None:
    ...
    keylog_list = tkinter.Listbox(keylog_window,
                                  activestyle='none',
...
welcome_msg = tkinter.Label(basic, font=('TkDefaultFont', 10), text="Welcome to Spam Bot GUI !")
...
class Clock:
    ...
    def get_update_thread(self) -> None:
        ...
        self.text = str(f"CPU: {psutil.cpu_percent(1)}% {time.strftime('%H:%M:%S', time.localtime())}")
...
file_item.add_checkbutton(label='Enable Spam Interval', onvalue=True, offvalue=False, variable=enable_spam_interval)
file_item.add_checkbutton(label='Enable Sound', onvalue=True, offvalue=False, variable=sound_enabled)
file_item.add_checkbutton(label='Pause the Clock', onvalue=True, offvalue=False, variable=Clock.enabled)
